...
frame (`unemp`), and the subsequent arguments are the columns to keep.

```{r, results = 'hide', purl = FALSE}
select(unemp, year, emp_tot, unemp)
```

To choose rows based on a specific criteria, use `filter()`:

```{r, purl = FALSE}
filter(unemp, year == 1995)
```
...
unemp %>%
  filter(unemp > 8000) %>%
  select(year, emp_tot, unemp)
```

In the above, we use the pipe to send the `unemp` dataset first through
`filter()` to keep rows where `unemp` is greater than 8000, then through `select()`
to keep only the `year`, `emp_tot`, and `unemp` columns. Since `%>%` takes
...
> ### Challenge {.challenge}
>
>  Using pipes, subset the `unemp` data to include individuals collected before
>  1995 and retain only the columns `year`, `emp_pct_pop`, and `party`.
...
unemp %>%
    filter(year < 1995) %>%
    select(year, emp_pct_pop, party)
```
--->

    ```{r, eval=FALSE, purl=TRUE, echo=FALSE}
    ## Pipes Challenge:
    ##  Using pipes, subset the data to include information collected
    ##  before 1995, and retain the columns `year`, `emp_pct_pop`, and `party`.
